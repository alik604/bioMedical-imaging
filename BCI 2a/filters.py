...
'''
    def bandpass_filter(signal_in,f_band_nom):
        # N channels with fir filter of order 101
        # Keyword arguments:
    ...
    def bandpass_filter(signal_in, f_band_nom):
        # N channels with bandpass filter
        # Keyword arguments:
        # signal_in -- numpy array of size [NO_channels, NO_samples]
        # f_band_nom -- normalized frequency band [freq_start, freq_end]
        # Return:	filtered signal
...
def butter_fir_filter(signal_in, filter_coeff): # used in CSP
    ''' 
...
def butter_bandpass_filter(data, lowcut, highcut, fs, order=5):
    ''' Butterworth filter with built-in bandpass filter, which rejects frequencies outside the given range 
    ...
    Keyword arguments:
        ...
        highcut -- 
        fs      -- 
        order   -- higher is steeper drop. sigmoid like shape 
