...
class riemannian_multiscale:
    ...
    def fit(self, data):
        ...
        for trial_idx in range(n_tr_trial):
            ...
            for temp_idx in range(self.n_temp):
                ...
                for freq_idx in range(self.n_freq):
                    ...
                    cov_mat[trial_idx, temp_idx, freq_idx] = 1 / (n_samples - 1) * np.dot(data_filter, np.transpose(
                        data_filter)) + self.rho / n_samples * np.eye(n_channel)
        ...
        for trial_idx in range(n_tr_trial):
            for temp_idx in range(self.n_temp):
                for freq_idx in range(self.n_freq):
                    train_feat[trial_idx, temp_idx, freq_idx] = self.riem_kernel(cov_mat[trial_idx, temp_idx, freq_idx],
    ...
    def onetrial_feature(self, data):
        ...
        for freq_idx in range(self.n_freq):
            ...
            # regularized covariance matrix
            cov_mat = 1 / (n_samples - 1) * np.dot(data_filter,
                                                   np.transpose(data_filter)) + self.rho / n_samples * np.eye(
                self.n_channel)
            #
